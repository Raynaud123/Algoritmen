...
public class ContainerStack extends JFrame {
    ...
    public void fillYard(Yard yard, JPanel panelCenter, int currentHeight) {
        int x_length = yard.matrix.length;
        int y_length = yard.matrix[0].length;
        ...
        for (int i=0; i<x_length; i++) {
            for (int j=0; j<y_length; j++) {
                ...
                BoxLayout boxLayout = new BoxLayout(panel[i][j], BoxLayout.X_AXIS);
                panel[i][j].setLayout(boxLayout);
                panel[i][j].setPreferredSize(new Dimension(10,10));
                panel[i][j].setBorder(BorderFactory.createLineBorder(Color.BLACK, 0));
